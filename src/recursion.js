...
var exponent = function(base, exp) {
  if (exp === 0) {
    ...
    return Number(((1 / base) * (1 / base) * exponent(base, exp + 2)).toFixed(5));
  }
};
...
var modulo = function(x, y) {
  if (y === 0) {
    ...
    return 0;
  } else if (y > x && x > 0 || y < x && x < 0 || y < x - x - x && x > 0 || y > x - x - x && x < 0) {
    return x;
  } else if (y === x  || x === y - y - y && x > 0) {
    return 0;
  } else if (y === x || x === y - y - y && x < 0) {
    return -0;
...
var compareStr = function(str1, str2) {
  if (str1.length !== str2.length) {
    ...
    return true;
  } else if (str1[0] === str2[0] && str1.length === 1) {
    return true;
...
var countValuesInObj = function(obj, value) {
  ...
  for (let property in obj) {
    ...
    if (typeof obj[property] === 'object') {
      result += countValuesInObj(obj[property], value);
    }
  }
...
var nestedEvenSum = function(obj) {
  ...
  for (let property in obj) {
    ...
    if (typeof obj[property] === 'object') {
      result += nestedEvenSum(obj[property]);
    }
  }
  return result;
};
...
var numToText = function(str) {
  ...
  if (str.length === 0) {
    ...
    if (str[0] in numKey) {
      return numKey[str[0]];
    } else if (!(str[0] in numKey)) {
      return str[0];
    }
  } else {
    if (str[0] in numKey) {
      return numKey[str[0]] + numToText(str.slice(1));
    } else if (!(str[0] in numKey)) {
...
var binarySearch = function(array, target, min, max) {
  ...
  var attempt = Math.floor((max + min) / 2);

  if (max <= min && array[attempt] !== target) {
    return null;
  } else if (array[attempt] === target) {
    return attempt;
  } else if (array[attempt] < target) {
    return binarySearch(array, target, attempt + 1,max);
  } else if (array[attempt] > target) {
    return binarySearch(array, target, min, attempt - 1);
  }
